...
| `RADIANCE_TOPK_TRITON_MIN_ROWS` | `1` | Row count at or above which vLLM's `apply_top_k_top_p` uses its Triton kernel instead of a vocabulary-wide `logits.sort()`. Stock vLLM hardcodes 8, on the reasoning that sorting a few rows is cheap; on gfx1201 at vocab 248320 the sort is slower at *every* row count -- Triton is flat at ~210 us out to 20 rows while the sort runs 235-1904 us, and the sort is not monotonic in rows because torch switches algorithm around 8 (which is why batch 8 profiles faster than batch 4 without this). Speculative decode sits under the gate: the rejection sampler calls it once per step on `batch x (num_speculative_tokens+1)` rows, and the bonus-token sampler again on 1. Measured on 2x R9700 toggling only this: **32.38 -> 31.47 ms/step (-2.8%)**, decode 78.6 -> 80.8 tok/s, acceptance identical at 1.544. The two paths are **bit-identical** -- same `-inf` mask, 0.0 maximum difference on the kept entries -- so this is scheduling, not numerics. Set `8` for stock behaviour. |
