...
| `RADIANCE_AR_MAX_KB` | `49152` | Size gate for the P2P all-reduce, in KB, and it has to track `--max-num-batched-tokens`. The gate compares the raw bf16 byte count, so a chunked-prefill all-reduce is `tokens x hidden x 2`: at the shipped 4096 and hidden 5120 that is 40 MiB and fits, but at 8192 it is 80 MiB -- over the cap -- and **every prefill reduction silently falls back to RCCL** while the P2P kernel only ever sees the small decode messages. Measured on 2x R9700 (TP2, Qwen3.8-27B) with a torch profile of a fixed 40k prompt: all-reduce was 18.8% of prefill GPU time on RCCL at 3.145 ms per call against 1.317 ms on the kernel (2.18x), worth **+0.9-7.3% prefill on fp8 and +3.1-12.8% on MXFP4**. KV cache size is unchanged -- the extra `2 x max_bytes` of IPC scratch comes out of non-KV budget. At chunk 16384 the message is 160 MiB and needs >= 196608. Verify with a torch profile: `ncclDevKernel` should be absent and the R4D all-reduce count should match `vllm::all_reduce`. |
| `RADIANCE_AR_QUANT_MIN_KB` | `128` | Message size below which the exact bf16 all-reduce kernel is used instead of the compressed one; compression only pays once the transfer is bandwidth-bound. Same default as before, now reachable. |
